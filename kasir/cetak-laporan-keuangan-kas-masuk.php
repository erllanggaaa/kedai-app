...
$nama_dokumen='Laporan Pemasukkan'; //Beri nama file PDF hasil.
...
<html>
    ...
    <body>
		...
		<div class="box box-info">
              ...
              <h5 class="box-title">Laporan Pemasukkan</h5>
		...
		<table width="100%" id="data" class="table table-bordered table-striped table-scalable">	
				...
				<tbody>
					...
					if(empty($_POST['tglawal']) && empty($_POST['tglakhir'])){
						...
						$no = 0;
						$sqltampillaporankasmasuk = mysqli_query($konek, "SELECT * FROM kas_masuk WHERE TGL_KAS_MASUK BETWEEN '".$_POST['tglawal']."' AND '".$_POST['tglakhir']."' ORDER BY TGL_KAS_MASUK ASC");
						while ($hasiltampillaporankasmasuk = mysqli_fetch_array($sqltampillaporankasmasuk)){
							$no++;
							echo "
								<tr>
						...
						$sqltotal = mysqli_query($konek, "SELECT SUM(TOTAL_KAS_MASUK) AS TOTAL FROM kas_masuk WHERE TGL_KAS_MASUK BETWEEN '".$_POST['tglawal']."' AND '".$_POST['tglakhir']."'");
						$hasiltotal = mysqli_fetch_array($sqltotal);
					}
					?>
				</tbody>
