; (function ($, window, document, undefined) {
	...
	defaults = {
	    template: '<div class="modal fade" tabindex="-1" role="dialog"><div class="modal-dialog modal-sm"><div class="modal-content"><div class="modal-header bg-{{type}} text-{{type}}"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button><h4 id="mySmallModalLabel" class="modal-title">{{title}}</h4></div><div class="modal-body"><form onsubmit="setTimeout(function () { window.location.reload(); }, 10)">{{message}}</form></div><div class="modal-footer"><button class="btn btn-danger" data-dismiss="modal">Batal</button><button class="btn btn-{{type}}" data-trigger="confirm">Ya</button></div></div></div></div>',
	    title: 'Confirm',
    ...
    $.extend(Plugin.prototype, {
        ...
        init: function () {
            ...
            if (nodeName == 'a') {
                var that = this;
                handler = function (e) {
                    window.location.href = 'data_Order.php';
                    $modal.modal('hide');
